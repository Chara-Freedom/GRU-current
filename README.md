...
2. The current version of the program should be specified via a special file (let's call it VERSION_1.0) inside the archive and in the directory with archives. This file should not be empty and should not contain spaces in its name for better parsing. Auto-update is implemented by comparing the VERSION file inside the archive and in the web directory with archives — it can be added to the code of any program, and it will connect to the main updater.
